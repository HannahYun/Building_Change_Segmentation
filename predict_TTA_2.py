...
if __name__ == '__main__':
    ...
    #! Load config
    config = load_yaml(os.path.join(prj_dir, 'config', 'predict_2.yaml'))
    train_config = load_yaml(os.path.join(prj_dir, 'results', 'train', config['train_serial'], 'train.yaml'))
    train_config_2 = load_yaml(os.path.join(prj_dir, 'results', 'train', config['train_serial_2'], 'train.yaml'))

    #! Set predict serial
    pred_serial = config['train_serial'] + '_' + datetime.now().strftime("%Y%m%d_%H%M%S") + '_2'
    ...
    # Load architecture2
    model2 = get_model(model_str=train_config_2['architecture'])
    if train_config_2['architecture'] == 'Lawin':
        model2.to(device)
    else:
        model2 = model2(
                    classes=train_config_2['n_classes'],
                    encoder_name=train_config_2['encoder'],
                    encoder_weights=train_config_2['encoder_weight'],
                    activation=train_config_2['activation']).to(device)
    logger.info(f"Load model2 architecture: {train_config_2['architecture']}")

    #! Load weight2
    check_point_path = os.path.join(prj_dir, 'results', 'train', config['train_serial_2'], 'model.pt')
    check_point = torch.load(check_point_path)
    model2.load_state_dict(check_point['model'])
    logger.info(f"Load model2 weight, {check_point_path}")

    # Save config
    save_yaml(os.path.join(pred_result_dir, 'train_config.yml'), train_config)
    save_yaml(os.path.join(pred_result_dir, 'train_config_2.yml'), train_config_2)
    ...
    with torch.no_grad():
        ...
        for batch_id, (x, x2, x3, x4, orig_size, filename) in enumerate(tqdm(test_dataloader)):
            ...
            y_pred5 = y_pred5.cpu().numpy()
            y_pred6 = y_pred6.cpu().numpy()
            y_pred7 = y_pred7.cpu().numpy()
            y_pred8 = y_pred8.cpu().numpy()
            ...
            for i in range(y_pred.shape[0]):
                for x in range(4):
                    ...
                    im1 = np.rot90(im1, 1)
                    im2 = np.rot90(im2, 1)
                    y_pred6[i][x] = cv2.hconcat([im1, im2])

                    im1 = y_pred7[i][x][:, :224]
                    im2 = y_pred7[i][x][:, 224:]
                    im1 = np.rot90(im1, 2)
                    im2 = np.rot90(im2, 2)
                    y_pred7[i][x] = cv2.hconcat([im1, im2])

                    im1 = y_pred8[i][x][:, :224]
                    im2 = y_pred8[i][x][:, 224:]
                    im1 = np.rot90(im1, 3)
                    im2 = np.rot90(im2, 3)
                    y_pred8[i][x] = cv2.hconcat([im1, im2])
